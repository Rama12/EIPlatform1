    ...
    <div className="p-6 lg:p-10 max-w-7xl mx-auto">
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-8">
        <div className="stat-card">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-surface-500 text-sm">Active Calls</span>
          </div>
          <p className="text-3xl font-display font-bold text-surface-900">2</p>
        </div>
        <div className="stat-card">
          <div className="flex items-center gap-3 mb-2">
          ...
          <p className="text-3xl font-display font-bold text-surface-900">12</p>
        </div>
        <div className="stat-card">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-surface-500 text-sm">Total Spent</span>
          </div>
          <p className="text-3xl font-display font-bold text-surface-900">$847</p>
        </div>
      </div>
      ...
      <section className="mb-10">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-display font-semibold text-surface-900">Active Calls</h2>
            ...
            >
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl bg-surface-100 flex items-center justify-center flex-shrink-0">
                  <call.icon className="w-6 h-6 text-surface-600" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <h3 className="font-semibold text-surface-900 truncate">{call.device}</h3>
                  ...
                  <p className="text-sm text-surface-500 mb-3">{call.issue}</p>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-sm">
                      <Wrench className="w-4 h-4 text-surface-400" />
                      <span className="text-surface-600">{call.technician}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <MapPin className="w-4 h-4 text-success-600" />
                      <span className="text-success-600 font-medium">ETA: {call.eta}</span>
                    </div>
                  </div>
                </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-display font-semibold text-surface-900">Recent History</h2>
          <Link to="/customer/history" className="text-sm text-primary-600 hover:text-primary-700 transition-colors flex items-center gap-1">
            View All
            ...
            <tbody>
              {recentHistory.map((call) => (
                <tr key={call.id}>
                  ...
                  <td className="font-medium text-surface-900">{call.device}</td>
                  <td className="text-surface-600">{call.issue}</td>
                  <td className="text-surface-500">{call.completedAt}</td>
                  <td>
                  ...
                  <td className="font-medium text-surface-900">{call.cost}</td>
                </tr>
              ))}
            </tbody>
          </table>
